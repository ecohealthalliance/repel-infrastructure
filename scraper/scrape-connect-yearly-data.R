...
all_years <- tibble(year = seq(from = 2005, to = current_year))
all_countries_years <- all_countries %>%
  crossing(all_years)

# Download and transform yearly variables ----------------------------------------------

# human migration - extend last reported values to present, do not assume 0s for NAs
...
human <- transform_human_migration(directory = dir_downloads) %>%
  group_split(country_destination, country_origin) %>%
  map_dfr(~na_interp(., "n_human_migrants", to_current = TRUE)) %>%
  right_join(all_countries_years) %>%
  mutate(imputed_value = ifelse(is.na(imputed_value), FALSE, imputed_value))
...
# human tourism - extend last reported values to present, do not assume 0s for NAs
...
tourism <- transform_tourism(directory = dir_downloads)  %>%
  group_split(country_destination, country_origin) %>%
  map_dfr(~na_interp(., "n_tourists", to_current = TRUE)) %>%
  right_join(all_countries_years) %>%
  mutate(imputed_value = ifelse(is.na(imputed_value), FALSE, imputed_value))
...
# livestock trade - extend last reported values to present, assume 0s for NAs
...
# ots trade dollars - extend last reported values to present, assume 0s for NAs
