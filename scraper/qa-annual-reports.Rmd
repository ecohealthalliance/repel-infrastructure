...
report_missing <- dbReadTable(conn, "annual_reports_status") %>%
  filter(!reported) %>%
  select(report, country_iso3c = code, report_year, semester, in_database = reported) %>%
...
test = animal_diseases %>%
  group_by(report, disease, oie_listed, serotype, disease_population) %>% 
  mutate(n = n()) %>%
...
%>%
  ...
  ungroup() %>%
  select(report, oie_listed, disease, disease_population, serotype, disease_status) %>%
  make_dt()
```
