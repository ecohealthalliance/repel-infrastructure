...
report_missing <- dbReadTable(conn, "annual_reports_status") %>%
  filter(!reported) %>%
  select(country_iso3c = code, report_year = year, semester, in_database = reported) %>%
...
animal_diseases %>%
  ...
  ungroup() %>%
  select(country, report_year, report_months, report_semester, oie_listed, disease, disease_population, serotype, status) %>%
  make_dt()
```
