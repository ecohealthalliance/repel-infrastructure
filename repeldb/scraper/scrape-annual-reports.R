...
reports_to_get <- left_join(annual_reports_status, annual_reports_metadata, by = c("country", "report_year", "semester")) %>%
...
report_resps <- map_if(report_resps, is.null,
                          function(x) list(report_status = "failed to fetch"))
...
reports_to_update <- reports_to_get %>%
  ...
  mutate(datetime_fetched = Sys.time(),
         fetched_status = map_chr(report_resps, "report_status"))
...
iwalk(annual_report_tables,
      ~update_sql_table(conn, paste0("annual_reports_", .y), .x,
