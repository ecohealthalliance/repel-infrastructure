...
cases_plots <- map(cases_df_split, function(z){

  ggplot(z, aes(x = yr, y = cases_coalesced, fill = status_coalesced, color = status_coalesced)) +
    #  geom_line() +
...
presence_plots <-  map(presence_df_split, function(z){

  ggplot(z, aes(x = yr, y = type, fill = missing)) +
...
plots <- map2(cases_plots, presence_plots, function( cases_plot, presence_plot){
