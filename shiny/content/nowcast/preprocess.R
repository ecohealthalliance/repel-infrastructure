...
cases_plots <- pred_df %>%
  ...
  future_map(function(z) {
   ggplot(z, aes(x = yr, y = cases_coalesced, color = status_coalesced)) +
        geom_line() +
...
names(cases_plots) <- labs

write_rds(cases_plots,  here::here("shiny", "content", "nowcast", "data", "cases_plots.rds"))
